...
def runner():
    ...
    # TODO: Change this to read on interrupt
    # Polling code from https://hackaday.com/2024/04/17/human-interfacing-devices-hid-over-i2c/
    while True:
        try:
...
- Touchpad currently does not work on Windows. Need to find out why it's not working and come up with a fix. 
- Current firmware code relies on polling. Touchpad has Interrupt pin, so I assume interrupt based report is possible. 
